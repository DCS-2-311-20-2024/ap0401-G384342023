...
function init() {
  ...
  function setScore(score) {
    ...
    document.getElementById("life").innerText = (life > 0)? "ooo".substring(0, life) : "-- Game Over --";
  }
  ...
  const paddle = new THREE.Group(); // パドルグループ
  {
    ...
    const right = new THREE.Mesh(
      new THREE.SphereGeometry(paddleR, nSeg, nSeg,0, Math.PI*2, Math.PI/2),
      new THREE.MeshPhongMaterial({ color: 0x666666, shininess: 100, specular: 0x404040 }));
  ...
  function paddleCheck() {
    if(Math.abs(ball.position.z - paddle.position.z) < paddleR + ballR &&
       Math.abs(ball.position.x - paddle.position.x) < paddleR/2 + ballR){
        ...
        if(ball.position.x > paddle.position.x + (paddleL)/2+paddleR/2){
          vx = Math.abs(vx);
        }else if(ball.position.x < -paddle.position.x -(paddleL)/2 - paddleR/2){
          vx = -Math.abs(vx);
        }
        if(nBrick <= 0){
          resetBrick();
  ...
  const bricks = new THREE.Group();
    ...
    for(let r=0; r< param.nRow; r++) {
      for (let c = 0; c < param.nCol; c++) {
        ...
        brick.position.set(
          (w+gapX)*(c-(param.nCol-1)/2),0,-(d+gapZ)*r
        )
